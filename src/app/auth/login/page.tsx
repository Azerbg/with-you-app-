  ...
  if (status === "loading" || status === "authenticated") {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="w-8 h-8 border-3 border-[#F5C400] border-t-transparent rounded-full animate-spin" />
      </div>
    );
  }
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                required
                className="w-full border border-[#6B5E44]/30 rounded-xl px-3 py-2.5 pr-10 text-sm text-[#5C3D00] bg-white focus:outline-none focus:border-[#F5C400] focus:ring-2 focus:ring-[#F5C400]/30 transition"
                placeholder="••••••••••"
              />
              <button
                type="button"
                onClick={() => setShowPassword(!showPassword)}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-[#9B8A6B] hover:text-[#5C3D00] transition"
                tabIndex={-1}
              >
                {showPassword ? (
