...
function StarRow({ rating }: { rating: number }) {
  return (
    <div className="flex">
      {[1, 2, 3, 4, 5].map(s => (
        <svg key={s} viewBox="0 0 20 20" className={`w-3 h-3 ${s <= Math.round(rating) ? "text-[#F5C400]" : "text-[#E8E0D4]"}`} fill="currentColor">
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/tutors/${tutorId}/reviews?skip=${reviews.length}&take=${PAGE_SIZE}`,
      );
      if (res.ok) {
        const next: Review[] = await res.json();
        setReviews(prev => [...prev, ...next]);
      }
    } finally {
      setLoading(false);
    }
  }, [tutorId, reviews.length]);
    ...
    <div className="mt-2">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        {reviews.map(r => {
          ...
          const initials = [r.student.firstName?.[0], r.student.lastName?.[0]]
            ...
            <div key={r.id} className="bg-[#FAF8F0] rounded-xl p-4 flex gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  ...
                  <StarRow rating={r.ratingComposite} />
